...
program
  ...
  .option("-rm, --remove_old_cache_locks", "Remove old cache locks before run task")
...
async function startTask() {
  ...
  process.on("SIGTERM", () => {
    printLog("info", `Received "SIGTERM" signal. Restarting seed and clean up...`);
  ...
  printLog("info", `Loading seed.json and cleanup.json files at "${opts.dataDir}"...`);

  if (!opts.cleanUp && !opts.seed) {
    printLog("info", `No seed or clean up task. Exited!`);
  }

  const [cleanUpData, seedData] = await Promise.all([readCleanUpFile(opts.dataDir), readSeedFile(opts.dataDir)]);
  ...
  if (opts.cleanUp) {
    try {
      ...
      for (const id in cleanUpData.datas) {
        try {
          await cleanXYZTileDataFiles(`${opts.dataDir}/caches/xyzs/${id}`, seedData.datas[id].metadata.format, cleanUpData.datas[id].zooms, cleanUpData.datas[id].bbox, seedData.datas[id].concurrency, seedData.datas[id].maxTry, cleanUpData.datas[id].cleanUpBefore?.time || cleanUpData.datas[id].cleanUpBefore?.day);
        } catch (error) {
  ...
  if (opts.seed) {
    try {
      ...
      for (const id in seedData.datas) {
        try {
          await seedXYZTileDataFiles(`${opts.dataDir}/caches/xyzs/${id}`, seedData.datas[id].metadata, seedData.datas[id].url, seedData.datas[id].bbox, seedData.datas[id].zooms, seedData.datas[id].concurrency, seedData.datas[id].maxTry, seedData.datas[id].timeout, seedData.datas[id].refreshBefore?.time || seedData.datas[id].refreshBefore?.day || seedData.datas[id].refreshBefore?.md5);
        } catch (error) {
