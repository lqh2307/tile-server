(function (global, factory) {
  typeof exports === "object" && typeof module !== "undefined"
    ...
    : ((global =
      ...
      (global.MaplibreInspect = factory()));
})(this, function () {
  "use strict";
  ...
  function renderProperty(propertyName, property) {
    return (
      `${
        '<div class="maplibregl-inspect_property">' +
        '<div class="maplibregl-inspect_property-name">'
      }${propertyName}</div>` +
      `<div class="maplibregl-inspect_property-value">${displayValue(
        property
      )}</div>` +
      "</div>"
    );
  ...
  function renderFeatures(features) {
    return features
      .map(
        (ft) =>
          `<div class="maplibregl-inspect_feature">${renderProperties(
            ft
          )}</div>`
      )
      .join("");
  ...
  function renderPopup(features) {
    return `<div class="maplibregl-inspect_popup">${renderFeatures(
      features
    )}</div>`;
  }
  ...
  class MaplibreInspect {
    constructor(options) {
      this._onSourceChange = (e) => {
        ...
        if (
          !isEqual(previousSources, this.sources) &&
    ...
    _inspectStyle() {
      const coloredLayers = stylegen.generateColoredLayers(
        this.sources,
